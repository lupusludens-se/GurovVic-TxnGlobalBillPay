...
export class TransactionBuilder {
    ...
    this.payload = {
      merchantId:
        data.merchantId !== undefined
          ? data.merchantId
          : faker.number.int({ min: 10000, max: 100000000 }),
      processorId:
      ...
      state: data.state !== undefined ? data.state : faker.location.state(),
      amount:
  ...
  withAmount(amount) {
    this.payload.amount = amount ?? faker.number.int({ min: 10, max: 1000 });
    return this;
  }
  withDueDate(dueDate) {
  ...
  withCustomerRef(customerRef) {
    const generateCustomRef = () => {
      const randomDigits = faker.number
        .int({ min: 100000000, max: 999999999 })
        .toString(); // 9 digits
      return `PGI ${randomDigits} - PO`; // Format string
    };
    this.payload.customerRef = customerRef ?? generateCustomRef();
    return this;
  }
  withPdfLink(pdfLink) {
  ...
  withInvoiceAmount(invoiceAmount) {
    // Ensure the invoiceAmount matches the amount
    this.payload.invoiceAmount = invoiceAmount ?? this.payload.amount;
    return this;
  }
